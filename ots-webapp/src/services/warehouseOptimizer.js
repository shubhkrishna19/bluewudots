  ...
  selectOptimalWarehouse(order, currentLoads = {}) {
    ...
    if (load < primaryWh.capacity * 0.9) {
      return {
        warehouse: primaryWh,
        reason: pincodeMatch
          ? `Closest hub by pincode (${primaryWh.name})`
          : `Regional hub (${primaryWh.name})`,
      }
    }
    ...
    const leastBusy = Object.values(this.warehouses).sort((a, b) => {
  ...
  getUtilizationMetrics(currentLoads = {}) {
    return Object.values(this.warehouses).map((wh) => {
      ...
      return {
        ...
        status:
          load / wh.capacity < 0.7 ? 'HEALTHY' : load / wh.capacity < 0.9 ? 'MODERATE' : 'HIGH',
        color: wh.color,
      }
    })
  }
