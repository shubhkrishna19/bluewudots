...
const VALID_TRANSITIONS = {
  ...
  [ORDER_STATUSES.QA_PASSED]: [ORDER_STATUSES.CARRIER_ASSIGNED, ORDER_STATUSES.CANCELLED],
  [ORDER_STATUSES.CARRIER_ASSIGNED]: [
  ...
  [ORDER_STATUSES.PICKED_UP]: [ORDER_STATUSES.IN_TRANSIT, ORDER_STATUSES.RTO_INITIATED],
  [ORDER_STATUSES.IN_TRANSIT]: [ORDER_STATUSES.OUT_FOR_DELIVERY, ORDER_STATUSES.RTO_INITIATED],
  [ORDER_STATUSES.OUT_FOR_DELIVERY]: [ORDER_STATUSES.DELIVERED, ORDER_STATUSES.RTO_INITIATED],
  [ORDER_STATUSES.DELIVERED]: [], // Terminal state
  [ORDER_STATUSES.RTO_INITIATED]: [ORDER_STATUSES.RTO_IN_TRANSIT],
  [ORDER_STATUSES.RTO_IN_TRANSIT]: [ORDER_STATUSES.RTO_DELIVERED],
  [ORDER_STATUSES.RTO_DELIVERED]: [
    ORDER_STATUSES.PENDING, // Re-ship
  ],
  [ORDER_STATUSES.CANCELLED]: [], // Terminal state
  [ORDER_STATUSES.ON_HOLD]: [ORDER_STATUSES.PENDING, ORDER_STATUSES.CANCELLED],
}
...
export const transitionOrder = async (order, newStatus, metadata = {}) => {
  ...
  if (!isValidTransition(currentStatus, newStatus)) {
    throw new Error(
      `Invalid transition from ${currentStatus} to ${newStatus}. Valid options: ${getValidNextStatuses(currentStatus).join(', ')}`
    )
  }
  ...
  if (newStatus === ORDER_STATUSES.LABEL_GENERATED && metadata.awb) {
    ...
    if (newStatus === ORDER_STATUSES.APPROVED) {
      // Assuming APPROVED is a status where Zoho sync happens
      try {
...
export const calculateOrderMetrics = (order) => {
  ...
  if (createdAt && pickedAt) {
    metrics.processingTime = Math.round(
      (new Date(pickedAt) - new Date(createdAt)) / (1000 * 60 * 60)
    ) // hours
  }

  if (pickedAt && deliveredAt) {
    metrics.transitTime = Math.round(
      (new Date(deliveredAt) - new Date(pickedAt)) / (1000 * 60 * 60 * 24)
    ) // days
  }

  if (createdAt && deliveredAt) {
    metrics.totalTime = Math.round(
      (new Date(deliveredAt) - new Date(createdAt)) / (1000 * 60 * 60 * 24)
    ) // days
  }
