...
class DataUtilsOptimized {
  ...
  static filterOrders(orders, criteria = {}) {
    return orders.filter(order => {
      ...
      if (criteria.startDate && new Date(order.orderDate) < new Date(criteria.startDate)) return false;
  ...
  static ordersToCSV(orders) {
    ...
    const headers = ['Order ID', 'Customer Name', 'Email', 'Amount', 'Order Date', 'Status', 'Source'];
