 ...
 */
export const createNotification = ({ type, title, message, data = {}, userId = null }) => {
...
export const notifyOrderShipped = (order) => {
  ...
  pushNotificationService.sendNotification(`Order Shipped: ${order.id}`, {
    body: `Carrier: ${order.carrier} | AWB: ${order.awb}`,
  })
...
export const notifyOrderDelivered = (order) => {
  ...
  pushNotificationService.sendNotification(`Order Delivered: ${order.id}`, {
    body: `Successfully delivered to ${order.city}`,
  })
