...
const PricingSimulator = ({ sku, getSKUProfitability }) => {
    ...
    return (
        <div className="pricing-simulator glass" style={{ padding: '20px', borderRadius: '12px', marginTop: '20px' }}>
            <h4 style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                🧮 Pricing Simulator: <span style={{ color: 'var(--primary)' }}>{sku.sku}</span>
            </h4>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                <div className="sim-inputs">
                    <label style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Simulated Selling Price (Inc GST)</label>
                    <input
                        type="range"
                ...
                <div className="sim-results glass" style={{ padding: '12px', background: 'rgba(255,255,255,0.02)' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                        ...
                        <span style={{ fontWeight: 'bold', color: prof?.profit > 0 ? 'var(--success)' : 'var(--danger)' }}>₹{prof?.profit}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span className="text-muted">Margin:</span>
                        <span style={{ fontWeight: 'bold', color: prof?.marginPercent > 15 ? 'var(--success)' : 'var(--warning)' }}>{prof?.marginPercent}%</span>
                    </div>
                </div>
            </div>
        </div>
...
const CommercialHub = () => {
    ...
    const filteredParents = selectedCategory === 'All'
        ? parents
    ...
    return (
        <div className="commercial-hub animate-fade">
            ...
            {selectedSkuForSim && (
                <PricingSimulator
                    sku={selectedSkuForSim}
                    getSKUProfitability={getSKUProfitability}
                />
            )}
            ...
            <div className="analytics-grid responsive-grid-2-1" style={{ marginTop: '32px' }}>
                    ...
                    <div className="lineage-list" style={{ marginTop: '20px' }}>
                        {filteredParents.map(parent => (
                            <div key={parent.sku} className="parent-node" style={{ marginBottom: '24px' }}>
                                <div className="parent-header glass" style={{ padding: '16px', borderRadius: '8px', borderLeft: '4px solid var(--primary)' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                        <h4 style={{ color: 'var(--primary)' }}>{parent.sku}</h4>
                                        <span className="badge" style={{ background: 'var(--bg-accent)', fontSize: '0.65rem' }}>PARENT MTP</span>
                                    </div>
                                    <p style={{ fontSize: '0.9rem', marginTop: '4px' }}>{parent.name}</p>
                                    <div style={{ display: 'flex', gap: '16px', marginTop: '12px', fontSize: '0.75rem' }} className="text-muted">
                                        ...
                                        return (
                                            <div
                                                ...
                                                style={{
                                                    ...
                                                    cursor: 'pointer',
                                                    background: selectedSkuForSim?.sku === child.sku ? 'rgba(99, 102, 241, 0.1)' : 'transparent'
                                                }}
                                                onClick={() => setSelectedSkuForSim(child)}
                                            >
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                    <div>
                                                        <span style={{ fontWeight: '600', fontSize: '0.85rem' }}>{child.sku}</span>
                                                        <span className="text-muted" style={{ fontSize: '0.75rem', marginLeft: '8px' }}>({child.colorFinish})</span>
                                                    </div>
                                                    <div style={{ textAlign: 'right' }}>
                                                        <span style={{ color: prof?.marginPercent > 15 ? 'var(--success)' : 'var(--warning)', fontWeight: '700' }}>
                                                            {prof?.marginPercent}% Margin
                                                        </span>
                                                    </div>
                                                </div>
                                                <div style={{ display: 'flex', gap: '12px', marginTop: '8px', fontSize: '0.7rem' }} className="text-muted">
                ...
                <div className="chart-card glass" style={{ padding: '24px' }}>
                    ...
                    <div className="benchmarks" style={{ marginTop: '32px' }}>
                        <h4 className="text-muted" style={{ fontSize: '0.75rem', marginBottom: '16px' }}>CATEGORY HEALTH BENCHMARKS</h4>
                        {filteredParents.slice(0, 5).map(p => (
                            <div key={p.sku} style={{ marginBottom: '16px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                                    ...
                                    <span className="text-muted" style={{ fontSize: '0.75rem' }}>Target: {p.categoryAvgMargin}%</span>
                                </div>
                    ...
                    <div className="commercial-insights glass" style={{ marginTop: '32px', padding: '16px', borderRadius: '8px' }}>
                        ...
                        <ul style={{ fontSize: '0.8rem', paddingLeft: '16px', color: 'var(--text-muted)' }}>
                            <li style={{ marginBottom: '8px' }}>High-weight SKUs are sensitive to <strong>Logistics</strong> rates.</li>
                            <li style={{ marginBottom: '8px' }}>Shoe Racks maintain <strong>higher margins</strong> due to low BOM vs SP ratio.</li>
                            <li>Event pricing targets a <strong>minimum 8-10% profit</strong> floor.</li>
                        </ul>
                    </div>
                </div>
            </div>
