...
const InvoiceGenerator = () => {
    ...
    return (
        <div className="invoice-generator animate-fade">
            ...
            {/* Summary Stats */}
            <div className="invoice-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginTop: '24px' }}>
                <div className="glass" style={{ padding: '20px', textAlign: 'center' }}>
                    <p style={{ fontSize: '2rem', fontWeight: '800', color: 'var(--primary)' }}>{mockInvoices.length}</p>
                    <span className="text-muted">Total Invoices</span>
                </div>
                <div className="glass" style={{ padding: '20px', textAlign: 'center' }}>
                    <p style={{ fontSize: '1.5rem', fontWeight: '800', color: 'var(--success)' }}>₹{totalRevenue.toLocaleString('en-IN')}</p>
                    <span className="text-muted">Total Revenue</span>
                </div>
                <div className="glass" style={{ padding: '20px', textAlign: 'center' }}>
                    <p style={{ fontSize: '1.5rem', fontWeight: '800', color: 'var(--accent)' }}>₹{totalGST.toLocaleString('en-IN')}</p>
            ...
            {/* Invoices Table */}
            <div className="invoices-table glass" style={{ marginTop: '32px', overflow: 'hidden', borderRadius: '12px' }}>
                <div className="table-header" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1.5fr 1fr 1fr 1fr 1fr', padding: '16px 20px', background: 'var(--bg-accent)', fontWeight: '700', fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>
                    <span>Invoice #</span>
                    <span>Order</span>
                    <span>Customer</span>
                    <span>Date</span>
            ...
            {selectedOrder && (
                <div className="modal-overlay" style={{
                    ...
                    <div className="invoice-preview glass animate-fade" style={{
                        width: '600px', maxHeight: '90vh', overflowY: 'auto', padding: '32px', borderRadius: '16px', background: '#fff', color: '#333'
                    }} onClick={(e) => e.stopPropagation()}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '2px solid #6366F1', paddingBottom: '16px' }}>
                            <div>
                            ...
                            <tbody>
                                {selectedOrder.items.map((item, idx) => (
                                    <tr key={idx} style={{ borderBottom: '1px solid #eee' }}>
                                        <td style={{ padding: '12px 0' }}>{item.name}<br /><span style={{ color: '#666', fontSize: '0.8rem' }}>{item.sku}</span></td>
                                        <td style={{ textAlign: 'right', padding: '12px 0' }}>{item.qty}</td>
                                        <td style={{ textAlign: 'right', padding: '12px 0' }}>₹{item.price.toLocaleString('en-IN')}</td>
                                        <td style={{ textAlign: 'right', padding: '12px 0' }}>{item.gst}%</td>
                                        <td style={{ textAlign: 'right', padding: '12px 0', fontWeight: '700' }}>₹{(item.price * (1 + item.gst / 100)).toLocaleString('en-IN')}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>

                        <div style={{ marginTop: '24px', textAlign: 'right', borderTop: '2px solid #6366F1', paddingTop: '16px' }}>
                            <p>Subtotal: ₹{selectedOrder.subtotal.toLocaleString('en-IN')}</p>
                            <p>GST (18%): ₹{selectedOrder.gstAmount.toLocaleString('en-IN')}</p>
                            <p style={{ fontSize: '1.5rem', fontWeight: '800', color: '#6366F1' }}>Total: ₹{selectedOrder.total.toLocaleString('en-IN')}</p>
                        </div>

                        <button
                            className="btn-primary"
                            style={{ width: '100%', marginTop: '24px', padding: '14px', background: '#6366F1', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer' }}
