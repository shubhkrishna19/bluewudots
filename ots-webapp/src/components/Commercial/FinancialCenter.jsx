...
const FinancialCenter = () => {
    ...
    return (
        <div className="financial-center-view animate-fade">
            ...
            <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px', marginTop: '32px' }}>
                <div className="stat-card glass" style={{ padding: '24px', borderTop: '4px solid var(--primary)' }}>
                    <span className="text-muted" style={{ fontSize: '0.75rem' }}>GROSS REVENUE</span>
                    <h2 style={{ marginTop: '8px' }}>{formatINR(finStats.totalRevenue)}</h2>
                    <p style={{ fontSize: '0.7rem', color: 'var(--success)', marginTop: '4px' }}>↑ 12% vs last month</p>
                </div>
                <div className="stat-card glass" style={{ padding: '24px', borderTop: '4px solid var(--warning)' }}>
                    <span className="text-muted" style={{ fontSize: '0.75rem' }}>TOTAL DEDUCTIONS</span>
                    <h2 style={{ marginTop: '8px' }}>{formatINR(finStats.totalCommissions + finStats.totalGst + finStats.totalOverhead + finStats.totalShipping)}</h2>
                    <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '4px' }}>Comms + GST + Ops + Ship</p>
                </div>
                <div className="stat-card glass" style={{ padding: '24px', borderTop: '4px solid var(--success)' }}>
                    <span className="text-muted" style={{ fontSize: '0.75rem' }}>NET REVENUE (EST)</span>
                    <h2 style={{ marginTop: '8px' }}>{formatINR(finStats.netRevenue)}</h2>
                    <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '4px' }}>Expected in Bank</p>
                </div>

                <div className="stat-card glass" style={{ padding: '24px', borderTop: `4px solid ${finStats.marginPercent > 15 ? 'var(--success)' : 'var(--danger)'}` }}>
                    <span className="text-muted" style={{ fontSize: '0.75rem' }}>NET MARGIN</span>
            ...
            <div className="financial-layout" style={{ marginTop: '40px', display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '32px' }}>
                            ...
                            <tbody>
                                {selectedView === 'overview' ? (
                                    orders.slice(0, 8).map(order => {
                                        ...
                                        const deductions = analysis.breakdown.tax + analysis.breakdown.commission + analysis.breakdown.overhead + analysis.breakdown.shipping;

                                        return (
                                            <tr key={order.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: '0.85rem' }}>
                                                <td style={{ padding: '12px' }}>{order.id} <br /><span className="text-muted" style={{ fontSize: '0.7rem' }}>{order.source}</span></td>
                                                <td style={{ padding: '12px' }}>{formatINR(order.amount)}</td>
                                                <td style={{ padding: '12px', color: 'var(--danger)' }}>- {formatINR(deductions)}</td>
                                                <td style={{ padding: '12px', fontWeight: '700' }}>{formatINR(analysis.netRevenue)}</td>
                                    ...
                                    settlements.map(set => (
                                        <tr key={set.orderId} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: '0.85rem' }}>
                                            <td style={{ padding: '12px' }}>{set.orderId}</td>
                                            <td style={{ padding: '12px' }}>{formatINR(set.amount)}</td>
                                            <td style={{ padding: '12px' }}>Marketplace</td>
                                            <td style={{ padding: '12px' }}>{new Date(set.timestamp).toLocaleDateString()}</td>
                                            <td style={{ padding: '12px' }}>
                                                <span className="badge" style={{
                                                    background: set.status === 'Matched' ? 'var(--success)' :
                                                        set.status === 'Discrepancy' ? 'var(--danger)' : 'var(--warning)',
                                                    fontSize: '0.6rem'
                                                }}>
                                                    {set.status}
                                                </span>
                ...
                <div className="breakdown-section" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    <div className="glass" style={{ padding: '24px' }}>
                    ...
                    <div className="glass highlight-border" style={{ padding: '24px', background: 'linear-gradient(135deg, rgba(99,102,241,0.1), rgba(139,92,246,0.1))' }}>
                        <h3>MTP Sync Status</h3>
                        <p className="text-muted" style={{ fontSize: '0.8rem', marginTop: '8px' }}>Legacy master data is currently active.</p>
                        <div style={{ marginTop: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                            <div className="pulse-icon" style={{ width: '12px', height: '12px', borderRadius: '50%', background: 'var(--success)', boxShadow: '0 0 10px var(--success)' }}></div>
                            <span style={{ fontSize: '0.9rem', fontWeight: '600' }}>Bluewud Legacy Link: ACTIVE</span>
                        </div>
                        <button className="btn-primary" style={{ width: '100%', marginTop: '20px', fontSize: '0.8rem' }}>Synchronize Nodes</button>
                    </div>
                </div>
            </div>
        </div>
