  ...
  return (
    <div className="global-ledger animate-fade">
      ...
      >
        ...
        <div
          className="currency-selector glass"
        ...
        >
          ...
          >
            {Object.keys(CURRENCY_DATABASE).map((code) => (
              <option key={code} value={code} style={{ background: 'var(--bg-main)' }}>
                {code} - {CURRENCY_DATABASE[code].name}
              </option>
            ))}
          </select>
        </div>
      ...
      >
        <div
          ...
          style={{ padding: '32px', borderLeft: '4px solid var(--primary)' }}
        >
          ...
          <p className="text-muted" style={{ marginTop: '8px' }}>
            Equivalent to ₹{totals.revenue.toLocaleString('en-IN')}
          </p>
        </div>

        <div
          className="stat-card glass"
          style={{ padding: '32px', borderLeft: '4px solid var(--success)' }}
        >
          <span className="text-muted" style={{ fontSize: '0.8rem' }}>
            NET GLOBAL PROFIT
          </span>
          <h1 style={{ fontSize: '2.5rem', marginTop: '10px', color: 'var(--success)' }}>
            {convertedProfit.formatted}
          </h1>
          <p className="text-muted" style={{ marginTop: '8px' }}>
            Equivalent to ₹{totals.profit.toLocaleString('en-IN')}
          </p>
        </div>
      </div>
