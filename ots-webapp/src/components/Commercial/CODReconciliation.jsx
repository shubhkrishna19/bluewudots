...
const CODReconciliation = () => {
  ...
  const codRemittances = orders
    ...
    .map((order) => {
      ...
      return {
        ...
        collectedDate: isDelivered
          ? new Date(order.lastUpdated || order.createdAt).toLocaleDateString()
          : null,
        remittanceDate: null,
        status: status,
  ...
  const pendingAmount = codRemittances
    .filter((c) => c.status !== 'Remitted')
    .reduce((sum, c) => sum + c.codAmount, 0)
  const overdueAmount = codRemittances
    ...
    <div className="cod-reconciliation animate-fade">
      ...
      >
        ...
        <span
          className="badge"
          style={{
            background: overdueAmount > 0 ? 'var(--danger)' : 'var(--success)',
            padding: '8px 16px',
            fontSize: '0.9rem',
          }}
        >
      ...
      >
        ...
        <div
          ...
          style={{
            padding: '24px',
            textAlign: 'center',
            borderTop: overdueAmount > 0 ? '4px solid var(--danger)' : 'none',
          }}
        >
      ...
      {/* Filters */}
      <div
        className="cod-filters glass"
        style={{ padding: '16px 20px', marginTop: '24px', display: 'flex', gap: '10px' }}
      >
            ...
            >
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <span
                    className="badge"
                    style={{ background: getStatusColor(cod.status), fontSize: '0.65rem' }}
                  >
                    {cod.status}
                  </span>
                  {cod.daysElapsed > 5 && (
