...
const SKUMaster = () => {
  ...
  const renderProfitMetric = (label, value, color = 'var(--text)') => (
    <div className="matrix-item">
      <span className="text-muted" style={{ fontSize: '0.7rem', textTransform: 'uppercase' }}>
        {label}
      </span>
      ...
      <div className="sku-grid responsive-card-grid" style={{ marginTop: '32px' }}>
        {filteredSKUs.map((sku, idx) => {
          ...
          return (
            <div
              key={idx}
              className="sku-card glass glass-hover animate-fade"
              style={{
                padding: '24px',
                borderLeft: `4px solid ${bauAnalysis.isHealthy ? 'var(--success)' : 'var(--warning)'}`,
              }}
            >
              <div
                style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
              >
                <div>
                  ...
                  <p className="text-muted" style={{ fontSize: '0.8rem' }}>
                    {enhancedSku.name}{' '}
                    <span style={{ opacity: 0.5 }}>({enhancedSku.parentSku || 'No Parent'})</span>
                  </p>
                </div>

                <div style={{ textAlign: 'right' }}>
                  <span
                    className={`badge ${(enhancedSku.tmsLevel || 'TL2').toLowerCase()}`}
                    style={{ fontSize: '0.6rem' }}
                  >
                    {enhancedSku.tmsLevel || 'TL2'}
                  </span>
                  <p className="text-muted" style={{ fontSize: '0.7rem', marginTop: '4px' }}>
                    {TMS_LEVELS[enhancedSku.tmsLevel || 'TL2']?.name}
                  </p>
                </div>
              </div>

              <div
                className="commercial-tiers"
                style={{
                  marginTop: '20px',
                  display: 'grid',
              ...
              >
                {/* BAU TIER */}
                <div
                  className="tier-box glass"
                  style={{ padding: '12px', background: 'rgba(255,255,255,0.03)' }}
                >
                  <span style={{ fontSize: '0.6rem', fontWeight: '800', opacity: 0.6 }}>
                    BAU (STANDARD)
                  </span>
                  <div style={{ marginTop: '8px' }}>
                    ...
                    <p
                      style={{
                        fontSize: '0.8rem',
                        color: bauAnalysis.isHealthy ? 'var(--success)' : 'var(--warning)',
                      }}
                    >
                ...
                >
                  ...
                  <div style={{ marginTop: '8px' }}>
                    <p style={{ fontSize: '1.1rem', fontWeight: '700' }}>
                      ₹{enhancedSku.eventPrice || 0}
                    </p>
                    <p
                      style={{
                        fontSize: '0.8rem',
                        color: eventAnalysis.isHealthy ? 'var(--success)' : 'var(--warning)',
                      }}
                    >
              ...
              {(() => {
                ...
                const stockOut = mlForecastService.predictStockOutDate(
                  orders,
                  enhancedSku.sku,
                  currentStock
                )
                ...
                return analysis && !analysis.error ? (
                  <div
                    className="ml-insight glass border-l-4 border-primary mt-4 overflow-hidden relative group"
                    style={{ padding: '16px', background: 'rgba(99, 102, 241, 0.03)' }}
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm font-bold m-0 leading-tight">
                          Forecasting{' '}
                          <span className="text-primary">+{analysis.metrics.growthRate}</span> units
                          next 30 days
                        </p>
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <span
                            className={`text-xs font-mono font-bold ${stockOut.days < 15 ? 'text-red-400' : 'text-green-400'}`}
                          >
                        ...
                        {stockOut && stockOut.urgency !== 'SAFE' && (
                          <div className="flex items-center gap-1.5 mt-3 animate-pulse">
                            ...
                            >
                              Stockout{' '}
                              {stockOut.date === 'OUT_OF_STOCK'
                                ? 'NOW'
                                : `Risk in ${stockOut.days} Days`}
                            </span>
                          </div>
                        )}
                      </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              <button className="btn-icon" onClick={() => setSelectedSKU(null)}>
                ✕
              </button>
            </div>
                  ...
                  <table style={{ width: '100%', fontSize: '0.85rem' }}>
                    ...
                    <tbody>
                      <tr>
                        <td>Amazon ASIN</td>
                        <td>
                          <code>{getEnhancedSKU(selectedSKU.sku, skuMaster).asin || 'N/A'}</code>
                        </td>
                      </tr>
                      <tr>
                        <td>Flipkart FSN</td>
                        <td>
                          <code>{getEnhancedSKU(selectedSKU.sku, skuMaster).fkfsn || 'N/A'}</code>
                        </td>
                      </tr>
                      <tr>
                        <td>Parent MTP</td>
                        <td>
                          <code>{getEnhancedSKU(selectedSKU.sku, skuMaster).parentSku}</code>
                        </td>
                      </tr>
                    </tbody>
                  </table>
            ...
            <div style={{ marginTop: '32px', textAlign: 'right' }}>
              <button className="btn-primary" onClick={() => setSelectedSKU(null)}>
                Close Insight
              </button>
            </div>
          </div>
