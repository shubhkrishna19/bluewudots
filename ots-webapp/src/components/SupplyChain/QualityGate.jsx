...
const QualityGate = () => {
  ...
  const pendingQA = useMemo(
    () =>
      orders.filter(
        (o) => o.status === ORDER_STATUSES.PENDING || o.status === ORDER_STATUSES.MTP_APPLIED
      ),
    [orders]
  )
  ...
  const handlePass = () => {
    ...
    updateOrderStatus(selectedOrder.id, ORDER_STATUSES.QA_PASSED, {
      reason: 'QA Checklist Verified',
    })
            ...
            <div style={{ marginTop: '32px', display: 'flex', gap: '16px' }}>
              <button
                ...
                style={{
                  flex: 2,
                  padding: '16px',
                  background: isPassed ? 'var(--success)' : 'var(--bg-accent)',
              ...
              <button
                ...
                onClick={() => {
                  updateOrderStatus(selectedOrder.id, ORDER_STATUSES.ON_HOLD, {
                    reason: 'QA Failed',
                  })
                  setSelectedOrder(null)
                  alert('Order Marked ON HOLD (QA Failed)')
                }}
              >
                ❌ FAIL
              </button>
