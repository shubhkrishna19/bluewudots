...
const ExportTools = () => {
  ...
  const generateInvoiceReport = () => {
    ...
    exportToCSV(invoiceData, 'Invoice_Report', [
      'orderId',
      'customer',
      'state',
      'sku',
  ...
  const generateCarrierReport = () => {
    ...
    orders.forEach((o) => {
      if (o.carrier) {
        ...
        if (['In-Transit', 'Out-for-Delivery', 'Picked-Up'].includes(o.status))
          carrierStats[o.carrier].inTransit++
      }
    })
  ...
  const reports = [
    ...
    },
    {
      ...
      icon: '⏳',
      data: orders.filter(
        (o) => !['Delivered', 'Cancelled'].includes(o.status) && !o.status.startsWith('RTO')
      ),
      count: orders.filter(
        (o) => !['Delivered', 'Cancelled'].includes(o.status) && !o.status.startsWith('RTO')
      ).length,
  ...
  const specialReports = [
    {
      ...
      icon: '🧾',
      action: generateInvoiceReport,
      description: 'GST-compliant invoice data',
    },
    {
      name: 'Carrier Performance',
      icon: '📊',
      action: generateCarrierReport,
      description: 'Delivery rate by carrier',
    },
    {
      name: 'Inventory Status',
      icon: '📦',
      action: generateInventoryReport,
      description: 'Stock levels with low-stock flags',
    },
  ]
  ...
  return (
    <div className="export-tools animate-fade">
      ...
      {exportStatus && (
        <div
          className={`status-banner animate-fade glass`}
          style={{
            padding: '12px 20px',
            marginTop: '16px',
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
          <div>
            <label
              ...
              style={{ fontSize: '0.75rem', display: 'block', marginBottom: '4px' }}
            >
              FROM
            </label>
            <input
          ...
          <div key={idx} className="export-card glass glass-hover" style={{ padding: '24px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button
                ...
                onClick={() =>
                  exportToCSV(report.data, report.name.replace(/\s/g, '_'), report.columns)
                }
              >
                📄 CSV
              </button>
