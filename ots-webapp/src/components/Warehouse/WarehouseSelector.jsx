            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {utilization.map(wh => {
                    ...
                    return (
                        <div
                            key={wh.id}
                            className={`glass glass-hover relative overflow-hidden transition-all duration-300 group ${isSelected ? 'ring-2 ring-primary shadow-lg shadow-primary/10' : 'border-white/5'
                                }`}
                            style={{
                                padding: '20px',
                                cursor: 'pointer',
                        ...
                        >
                            ...
                            <div className="flex justify-between items-start mb-4">
                                <div className="flex gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] opacity-60 uppercase tracking-tighter mt-1">{wh.name}</p>
                                    </div>
                                </div>
                                <div
                                    className="px-2 py-0.5 rounded-md text-[9px] font-bold tracking-wider uppercase bg-white/5"
                                    style={{ color: getStatusColor(wh.status), border: `1px solid ${getStatusColor(wh.status)}22` }}
                                >
                                    {wh.status}
                                </div>
                            </div>
                            ...
                            <div className="mt-2">
                                <div className="flex justify-between items-end mb-1.5 px-0.5">
                                    <span className="text-[10px] uppercase font-bold opacity-40">Load Utilization</span>
                                    <span className="text-xs font-black tracking-tight" style={{ color: getStatusColor(wh.status) }}>
                                        {wh.utilization}%
                                    </span>
                                </div>
                                ...
                                <div className="flex justify-between mt-3 px-1">
                                    <div className="flex items-center gap-1 opacity-50">
                                        ...
                                        <span className="text-[9px] font-medium tracking-tight">Available Cap: {wh.available.toLocaleString()}</span>
                                    </div>
                                    {isSelected && <CheckCircle className="w-4 h-4 text-primary animate-in zoom-in" />}
                                </div>
                            </div>
