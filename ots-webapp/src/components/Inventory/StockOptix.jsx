    ...
    <div className="stock-optix animate-fade">
      <div className="section-header">
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <span className="badge" style={{ background: 'var(--accent)', fontSize: '1.2rem' }}>
            {skuMaster.length}
          </span>
        </div>
      </div>
      ...
      >
        <div className="glass p-4 text-center border-l-4 border-red-500">
          <h3 className="text-2xl font-bold text-red-400">
            {inventoryHealth.filter((i) => i.status === 'STOCKOUT').length}
          </h3>
        ...
        <div className="glass p-4 text-center border-l-4 border-yellow-500">
          <h3 className="text-2xl font-bold text-yellow-400">
            {inventoryHealth.filter((i) => i.status === 'CRITICAL').length}
          </h3>
        ...
        <div className="glass p-4 text-center border-l-4 border-blue-500">
          <h3 className="text-2xl font-bold text-blue-400">
            {inventoryHealth.filter((i) => i.status === 'LOW').length}
          </h3>
          <p className="text-xs text-slate-400 uppercase tracking-wider mt-1">
            Low Supply ({'<'}15 Days)
          </p>
        ...
        <div className="glass p-4 text-center border-l-4 border-green-500">
          <h3 className="text-2xl font-bold text-green-400">
            {inventoryHealth.filter((i) => i.status === 'HEALTHY').length}
          </h3>
      ...
      <div
        ...
        style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px', marginTop: '32px' }}
      >
              ...
              <tbody>
                {inventoryHealth.map((item) => (
                  ...
                  >
                    ...
                    <td className="p-3">
                      <span
                        className="px-2 py-1 rounded text-[10px] font-bold border border-current"
                    ...
                    <td className="p-3 rounded-r-lg">
                      <button className="text-xs bg-slate-800 hover:bg-slate-700 px-3 py-1 rounded transition-colors text-white border border-white/5">
                        Analyze
                      </button>
                    </td>
                  </tr>
        ...
        <div className="analysis-panel space-y-6">
          {selectedSKU ? (
            <>
              {/* SKU Details */}
              <div
                className="glass p-6 border-t-4"
                style={{ borderColor: getStatusColor(selectedSKU.status) }}
              >
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  <div className="bg-slate-900/50 p-3 rounded">
                    ...
                    <p className="text-lg font-mono text-red-400">
                      {selectedSKU.daysOfSupply < 99
                        ? `in ${selectedSKU.daysOfSupply} days`
                        : 'Stable'}
                    </p>
                  </div>
                </div>
                ...
                {transferSuggestions.length > 0 ? (
                  <div className="space-y-3">
                    {transferSuggestions.map((t, idx) => (
                      <div
                        key={idx}
                        className="bg-slate-900/40 p-3 rounded border border-white/5 flex justify-between items-center group hover:border-accent/50 transition-colors"
                      >
                        <div>
                        ...
                        <div className="text-right">
                          ...
                          <button className="text-[10px] text-accent underline mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            Approve
                          </button>
                        </div>
                      </div>
