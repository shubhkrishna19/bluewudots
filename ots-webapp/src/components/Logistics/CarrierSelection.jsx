...
import { getAllRates, getRecommendation, getZone, CARRIER_RATES } from '../../services/carrierRateEngine';
...
const CarrierSelection = () => {
    ...
    const pendingAssignment = orders.filter(o =>
        ['Pending', 'MTP-Applied'].includes(o.status) && !o.carrier
    );
    ...
    return (
        <div className="carrier-view animate-fade">
            <div className="section-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px' }}>
                <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '16px' }}>
                    <div>
                        ...
                        <input
                            ...
                            style={{ width: '100%', padding: '10px', background: 'var(--bg-accent)', border: '1px solid var(--glass-border)', borderRadius: '6px', color: '#fff' }}
                        />
                    </div>
                    <div>
                        <label className="text-muted" style={{ fontSize: '0.75rem', display: 'block', marginBottom: '6px' }}>STATE</label>
                        <select
                    ...
                    <div>
                        <label className="text-muted" style={{ fontSize: '0.75rem', display: 'block', marginBottom: '6px' }}>CITY</label>
                        <input
                            type="text"
                            value={shipment.city}
                    ...
                    <div>
                        <label className="text-muted" style={{ fontSize: '0.75rem', display: 'block', marginBottom: '6px' }}>PAYMENT</label>
                    ...
                    {shipment.isCOD && (
                        <div>
                            <label className="text-muted" style={{ fontSize: '0.75rem', display: 'block', marginBottom: '6px' }}>COD AMOUNT</label>
                            <input
                                type="number"
                    ...
                    <div>
                        <label className="text-muted" style={{ fontSize: '0.75rem', display: 'block', marginBottom: '6px' }}>OPTIMIZE FOR</label>
                        <select
                            value={priority}
            ...
            <div className="rate-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                {/* Recommended Carrier */}
                <div className="recommendation-hero glass" style={{ padding: '32px', position: 'relative', overflow: 'hidden' }}>
                    <div style={{ position: 'absolute', top: '0', right: '0', background: 'var(--success)', padding: '6px 16px', fontSize: '0.75rem', fontWeight: '700' }}>
                        ✨ RECOMMENDED
                    </div>
                    <span className="badge" style={{ background: 'var(--primary)' }}>{recommendation.recommendation}</span>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginTop: '24px' }}>
                        <div>
                            ...
                            <p style={{ fontSize: '2.5rem', fontWeight: '800', color: 'var(--success)' }}>₹{recommendation.total}</p>
                        </div>
                        <div>
                            <span className="text-muted" style={{ fontSize: '0.75rem' }}>DELIVERY</span>
                    ...
                    <div className="glass" style={{ padding: '16px', marginTop: '20px' }}>
                        <p className="text-muted" style={{ fontSize: '0.75rem', marginBottom: '12px' }}>COST BREAKDOWN</p>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '0.85rem' }}>
                            <span>Freight:</span><span style={{ textAlign: 'right' }}>₹{recommendation.breakdown?.freight}</span>
                            <span>Fuel Surcharge:</span><span style={{ textAlign: 'right' }}>₹{recommendation.breakdown?.fuelSurcharge}</span>
                            {shipment.isCOD && <><span>COD Charge:</span><span style={{ textAlign: 'right' }}>₹{recommendation.breakdown?.codCharge}</span></>}
                            <span>GST (18%):</span><span style={{ textAlign: 'right' }}>₹{recommendation.breakdown?.gst}</span>
                            <span style={{ fontWeight: '700', borderTop: '1px solid var(--glass-border)', paddingTop: '8px' }}>Total:</span>
                            <span style={{ fontWeight: '700', textAlign: 'right', borderTop: '1px solid var(--glass-border)', paddingTop: '8px', color: 'var(--success)' }}>₹{recommendation.total}</span>
                        </div>
                    </div>
                </div>
                            ...
                            >
                                <div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        ...
                                        {idx === 0 && <span className="badge" style={{ background: 'var(--success)', fontSize: '0.6rem', padding: '2px 6px' }}>BEST</span>}
                                    </div>
                                ...
                                <div style={{ textAlign: 'right' }}>
                                    <p style={{ fontSize: '1.3rem', fontWeight: '700', color: idx === 0 ? 'var(--success)' : '#fff' }}>₹{rate.total}</p>
                                    {idx > 0 && (
                                        <span className="text-muted" style={{ fontSize: '0.75rem' }}>
                                            +₹{rate.total - rates[0].total} ({((rate.total / rates[0].total - 1) * 100).toFixed(0)}%)
                                        </span>
                                    )}
                                </div>
                            </div>
                            ...
                            <div key={order.id} className="glass glass-hover" style={{ padding: '20px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                                    <div>
                                        ...
                                        <p className="text-muted" style={{ fontSize: '0.85rem' }}>{order.customerName}</p>
                                    </div>
                                    <span className="badge" style={{ background: 'var(--warning)', fontSize: '0.65rem' }}>{order.status}</span>
                                </div>
                                <p style={{ fontSize: '0.85rem' }}>{order.city}, {order.state} • {order.weight}kg</p>
                                <button
