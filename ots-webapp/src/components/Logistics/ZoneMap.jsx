...
const INDIA_ZONES = {
    'North': {
        states: ['Delhi', 'Haryana', 'Punjab', 'Uttar Pradesh', 'Uttarakhand', 'Himachal Pradesh', 'Jammu and Kashmir', 'Ladakh', 'Chandigarh'],
        color: '#3B82F6'
    },
    'South': {
    ...
    'East': {
        states: ['West Bengal', 'Odisha', 'Bihar', 'Jharkhand', 'Sikkim', 'Assam', 'Arunachal Pradesh', 'Nagaland', 'Manipur', 'Mizoram', 'Tripura', 'Meghalaya'],
        color: '#F59E0B'
    },
    'West': {
...
const ZoneMap = () => {
    ...
    return (
        <div className="zone-map animate-fade">
            ...
            {/* Zone Cards */}
            <div className="zone-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px', marginTop: '32px' }}>
                {Object.entries(INDIA_ZONES).map(([zone, data]) => (
                    <div key={zone} className="zone-card glass glass-hover" style={{ padding: '24px', borderTop: `4px solid ${data.color}` }}>
                        ...
                        <div style={{ marginTop: '20px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                                ...
                                <span style={{ color: 'var(--success)', fontWeight: '700' }}>{zoneStats[zone]?.delivered || 0}</span>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <span className="text-muted">In Transit</span>
                                <span style={{ color: 'var(--info)', fontWeight: '700' }}>{zoneStats[zone]?.inTransit || 0}</span>
                            </div>
                        </div>
            ...
            <div className="top-states glass" style={{ marginTop: '32px', padding: '24px' }}>
                ...
                <div style={{ marginTop: '20px' }}>
                    {sortedStates.map(([state, count], idx) => (
                        <div key={state} style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
                            <span style={{
                                width: '32px',
                                height: '32px',
                                borderRadius: '50%',
            ...
            {/* Zone Legend */}
            <div className="zone-legend glass" style={{ marginTop: '24px', padding: '20px', display: 'flex', justifyContent: 'center', gap: '32px' }}>
                {Object.entries(INDIA_ZONES).map(([zone, data]) => (
                    <div key={zone} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <div style={{ width: '16px', height: '16px', borderRadius: '4px', background: data.color }}></div>
                        <span>{zone} Zone</span>
                    </div>
                ))}
            </div>
