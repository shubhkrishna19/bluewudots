...
const OrderImport = () => {
  ...
  const executeImport = async () => {
    const validOrders = previewData.filter((r) => r.validation.valid).map((r) => r.order)
  ...
  const downloadTemplate = () => {
    const templates = {
      amazon:
        'order-id,buyer-name,buyer-phone-number,ship-address-1,ship-city,ship-state,ship-postal-code,sku,quantity-purchased,item-price\nAMZ-001,John Doe,9876543210,123 Main St,Mumbai,Maharashtra,400001,SKU-001,1,1500',
      flipkart:
        'order_item_id,buyer_name,buyer_phone,ship_address,ship_city,ship_state,ship_pincode,sku,quantity,selling_price\nFKT-001,Jane Doe,9876543211,456 Oak Ave,Delhi,Delhi,110001,SKU-002,2,2500',
      shopify:
        'order_number,first_name,last_name,phone,address1,city,province,zip,sku,quantity,total_price\n1001,Bob,Smith,9876543212,789 Pine Rd,Bangalore,Karnataka,560001,SKU-003,1,3000',
      generic:
        'customerName,phone,address,city,state,pincode,sku,quantity,weight,amount\nCustomer Name,9876543213,Address Line,City,State,123456,SKU-004,1,2.5,2000',
    }
  ...
  return (
    <div className="order-import animate-fade">
      ...
      {importStatus && importStatus.type === 'success' && (
        <div
          className="success-banner glass animate-fade"
          style={{
            padding: '20px',
            marginTop: '20px',
        ...
        >
          <div>
            <p style={{ fontWeight: '700', color: 'var(--success)' }}>✅ Import Successful!</p>
            <p className="text-muted">
              {importStatus.imported} orders imported
              {importStatus.failed > 0 && `, ${importStatus.failed} skipped`}
            </p>
          </div>
          <button className="btn-secondary" onClick={() => setImportStatus(null)}>
            Dismiss
          </button>
        </div>
      )}
      ...
      {errors.length > 0 && (
        <div
          className="error-banner glass animate-fade"
          style={{
            padding: '16px 20px',
            marginTop: '20px',
        ...
        >
          {errors.map((err, i) => (
            <p key={i} style={{ color: 'var(--danger)' }}>
              ⚠️ {err}
            </p>
          ))}
        </div>
          ...
          {sources.map((source) => (
            <div
              ...
              style={{
                ...
                background:
                  selectedSource === source.id ? 'rgba(99, 102, 241, 0.15)' : 'transparent',
              }}
      ...
      {previewData.length > 0 && (
        <div className="preview-section glass" style={{ padding: '24px', marginTop: '24px' }}>
          ...
          >
            <h4>
              📋 Preview ({previewData.filter((r) => r.validation.valid).length} valid /{' '}
            ...
            >
              {isProcessing
                ? '⏳ Importing...'
                : `✅ Import ${previewData.filter((r) => r.validation.valid).length} Orders`}
            </button>
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--glass-border)' }}>
                  ...
                  <th style={{ padding: '12px', textAlign: 'left', fontSize: '0.8rem' }}>
                    CUSTOMER
                  </th>
