    ...
    <div className="dealer-order-entry animate-fade">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="text-right">
          ...
          <p
            className={`text-xl font-bold ${creditStatus?.success === false ? 'text-red-400' : 'text-green-400'}`}
          >
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          ...
          {message && (
            <div
              className={`p-4 rounded-lg flex gap-3 ${message.type === 'success' ? 'bg-green-500/10 border border-green-500/30 text-green-300' : 'bg-red-500/10 border border-red-500/30 text-red-300'}`}
            >
          ...
          <button
            ...
            className={`w-full p-4 rounded-lg font-bold flex items-center justify-center gap-2 transition-all ${
              isSubmitting || !selectedSku || (creditStatus && !creditStatus.success)
        ...
        <div className="space-y-6">
          ...
          <div className="glass p-6">
            ...
            <div className="space-y-2 text-xs">
              <div
                className={`flex justify-between p-2 rounded ${quantity >= 100 ? 'bg-purple-500/20 text-purple-200' : 'text-slate-500'}`}
              >
                <span>100+ Units</span>
                <span>20% OFF</span>
              </div>
              <div
                className={`flex justify-between p-2 rounded ${quantity >= 50 && quantity < 100 ? 'bg-purple-500/20 text-purple-200' : 'text-slate-500'}`}
              >
                <span>50 - 99 Units</span>
                <span>15% OFF</span>
              </div>
              <div
                className={`flex justify-between p-2 rounded ${quantity >= 10 && quantity < 50 ? 'bg-purple-500/20 text-purple-200' : 'text-slate-500'}`}
              >
                <span>10 - 49 Units</span>
                <span>10% OFF</span>
              </div>
              <div
                className={`flex justify-between p-2 rounded ${quantity < 10 ? 'bg-slate-700/50 text-slate-300' : 'text-slate-500'}`}
              >
                <span>Default</span>
