...
const MOCK_DEALERS = [
    { id: 'DLR-001', name: 'Furniture World', code: 'FW-BLR', city: 'Bangalore', state: 'Karnataka', type: 'Distributor', creditLimit: 500000, active: true },
    { id: 'DLR-002', name: 'Home Decor Hub', code: 'HDH-MUM', city: 'Mumbai', state: 'Maharashtra', type: 'Retailer', creditLimit: 250000, active: true },
    { id: 'DLR-003', name: 'Lifestyle Furnish', code: 'LF-DEL', city: 'Delhi', state: 'Delhi', type: 'Franchise', creditLimit: 750000, active: true },
    { id: 'DLR-004', name: 'Comfort Zone', code: 'CZ-CHN', city: 'Chennai', state: 'Tamil Nadu', type: 'Retailer', creditLimit: 200000, active: false },
    { id: 'DLR-005', name: 'Urban Living', code: 'UL-HYD', city: 'Hyderabad', state: 'Telangana', type: 'Distributor', creditLimit: 400000, active: true }
];
        ...
        <div className="dealer-module animate-fade">
            ...
            <div className="dealer-controls glass" style={{ padding: '20px', marginTop: '24px', display: 'flex', gap: '16px', alignItems: 'center' }}>
                ...
                <select
                    ...
                    style={{ padding: '12px 16px', background: 'var(--bg-accent)', border: '1px solid var(--glass-border)', borderRadius: '8px', color: '#fff' }}
                >
            ...
            <div className="dealer-grid" style={{ display: 'grid', gridTemplateColumns: selectedDealer ? '1fr 1fr' : '1fr', gap: '24px', marginTop: '24px' }}>
                        ...
                        >
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                <div>
                                    ...
                                    <p className="text-muted" style={{ fontSize: '0.8rem' }}>{dealer.code}</p>
                                </div>
                                <span className="badge" style={{
                                    background: dealer.type === 'Distributor' ? 'var(--primary)' :
                                        dealer.type === 'Retailer' ? 'var(--accent)' : 'var(--success)',
                                    fontSize: '0.65rem'
                                }}>
                                    {dealer.type}
                                </span>
                            </div>
                            <p style={{ marginTop: '12px', fontSize: '0.85rem' }}>{dealer.city}, {dealer.state}</p>
                        </div>
                    ))}
                ...
                {selectedDealer && (
                    <div className="dealer-detail glass animate-fade" style={{ padding: '28px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
                            <div>
                                <span className="text-muted" style={{ fontSize: '0.7rem' }}>DEALER PROFILE</span>
                                <h3>{selectedDealer.name}</h3>
                            </div>
                            <span className={`badge ${selectedDealer.active ? '' : 'inactive'}`} style={{ background: selectedDealer.active ? 'var(--success)' : 'var(--danger)' }}>
                        ...
                        <div className="detail-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                            <div className="detail-item">
                                <span className="text-muted" style={{ fontSize: '0.7rem' }}>DEALER CODE</span>
                                <p style={{ fontWeight: '700', fontSize: '1.1rem' }}>{selectedDealer.code}</p>
                            </div>
                            <div className="detail-item">
                                <span className="text-muted" style={{ fontSize: '0.7rem' }}>TYPE</span>
                                <p style={{ fontWeight: '700' }}>{selectedDealer.type}</p>
                            </div>
                            <div className="detail-item">
                                <span className="text-muted" style={{ fontSize: '0.7rem' }}>LOCATION</span>
                                <p>{selectedDealer.city}, {selectedDealer.state}</p>
                            </div>
                            <div className="detail-item">
                                <span className="text-muted" style={{ fontSize: '0.7rem' }}>CREDIT LIMIT</span>
                                <p style={{ fontWeight: '700', color: 'var(--success)' }}>{formatINR(selectedDealer.creditLimit)}</p>
                            </div>
                        </div>
                        ...
                        <p className="text-muted" style={{ marginTop: '20px', fontSize: '0.7rem', textAlign: 'center' }}>
