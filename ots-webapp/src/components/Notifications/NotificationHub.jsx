...
const NotificationHub = ({ isOpen, onClose }) => {
    ...
    const seedDemoNotifications = () => {
        createNotification({ type: 'ORDER_DELIVERED', title: 'Order Delivered', message: 'BW-9901 delivered to Mumbai', data: { orderId: 'BW-9901', channel: 'orders' } });
        createNotification({ type: 'ORDER_SHIPPED', title: 'Order Shipped', message: 'BW-9902 picked up by BlueDart. AWB: BD987654321', data: { orderId: 'BW-9902', channel: 'orders' } });
        createNotification({ type: 'CARRIER_ISSUE', title: 'Carrier Delay', message: 'Delhivery shipments delayed in North Zone', data: { carrier: 'Delhivery', channel: 'alerts' } });
        createNotification({ type: 'BULK_IMPORT', title: 'Bulk Import Complete', message: '12 orders from Amazon', data: { count: 12, channel: 'system' } });
        createNotification({ type: 'COD_PENDING', title: 'COD Remittance', message: '5 COD orders pending (₹45,000)', data: { count: 5, channel: 'alerts' } });
        createNotification({ type: 'ORDER_RTO', title: 'RTO Initiated', message: 'BW-9856 returned', data: { orderId: 'BW-9856', channel: 'orders' } });
        createNotification({ type: 'LOW_STOCK', title: 'Low Stock Alert', message: 'BL-DESK-01 below reorder level', data: { sku: 'BL-DESK-01', channel: 'alerts' } });
        createNotification({ type: 'SYSTEM_ALERT', title: 'WhatsApp Sent', message: 'Order confirmation sent to +91 9876543210', data: { channel: 'whatsapp' } });
    };
    ...
    const filteredNotifications = useMemo(() => {
        return notifications.filter(n => {
            ...
            if (activeChannel === 'orders') return n.type?.includes('order') || n.data?.channel === 'orders';
            if (activeChannel === 'alerts') return n.priority === 'critical' || n.priority === 'high' || n.data?.channel === 'alerts';
    ...
    return (
        <div className="notification-hub-overlay animate-fade" style={{
            position: 'fixed',
            top: 0,
            right: 0,
            bottom: 0,
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        ...
                        <p className="text-muted" style={{ fontSize: '0.8rem' }}>WhatsApp • Push • System Alerts</p>
                    </div>
                    <button
                        onClick={onClose}
                    ...
                    {Object.values(CHANNELS).map(channel => (
                        <button
                            ...
                            style={{
                                ...
                                background: activeChannel === channel.key
                                ...
                                boxShadow: activeChannel === channel.key
