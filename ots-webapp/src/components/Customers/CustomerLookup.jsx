  ...
  const filteredCustomers = useMemo(() => {
    ...
    if (showAtRiskOnly) {
      customers = customers.filter(
        (c) => c.churn.riskLevel === 'HIGH' || c.churn.riskLevel === 'CRITICAL'
      )
    }
    ...
    <div className="customer-lookup animate-fade">
      ...
      >
        ...
        <button
          ...
          style={{
            padding: '12px 20px',
            background: showAtRiskOnly ? 'var(--danger)' : 'var(--bg-accent)',
          }}
                  ...
                  <div>
                    ...
                    >
                      <span
                        ...
                        style={{
                          background:
                            customer.churn.riskLevel === 'CRITICAL'
                              ? 'var(--danger)'
                              : customer.churn.riskLevel === 'HIGH'
                                ? 'var(--warning)'
                                : customer.churn.riskLevel === 'MEDIUM'
                                  ? 'var(--accent)'
                                  : 'var(--success)',
                          fontSize: '0.6rem',
                        }}
                      >
                        {customer.churn.riskLevel}
                      </span>
                      <span
                        className="badge"
                        style={{
                    ...
                    {(customer.churn.riskLevel === 'HIGH' ||
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation()
                          sendReactivationMessage(customer)
                        }}
                      >
                        📨 Reactivate
                      </button>
              ...
              <div className="customer-detail glass animate-fade" style={{ padding: '28px' }}>
                ...
                >
                  <div>
                    ...
                    <p className="text-muted">
                      📞 {redactPII(selectedCustomer.phone, user?.role)} | ✉️{' '}
                      {redactPII(selectedCustomer.email, user?.role)}
                    </p>
                  </div>
                  <button
