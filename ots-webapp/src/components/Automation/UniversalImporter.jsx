...
const CHANNEL_CONFIGS = {
  ...
  shopify: {
    name: 'Shopify',
    idField: 'Name',
    customerField: 'Billing Name',
    cityField: 'Shipping City',
  ...
  dealer: {
    name: 'Dealer',
    idField: 'order_id',
    customerField: 'dealer_name',
    cityField: 'city',
    stateField: 'state',
    skuField: 'sku',
  },
  custom: {
    name: 'Auto-Detect (Smart Map)',
    idField: null,
    customerField: null,
    cityField: null,
    stateField: null,
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        if (selectedChannel === 'custom') {
          ...
          config.idField =
            headers.find((h) => SMART_MAPPINGS.id.includes(h.toLowerCase())) || headers[0]
          config.customerField =
            headers.find((h) => SMART_MAPPINGS.customer.includes(h.toLowerCase())) || headers[1]
        ...
        const transformed = rawData.map((row) => ({
          id:
            row[config.idField] || `BW-CS-${Math.random().toString(36).substr(2, 5).toUpperCase()}`,
    ...
    <div className="importer-view animate-fade">
      ...
      {!selectedChannel ? (
        ...
        <div className="import-workspace" style={{ marginTop: '32px' }}>
          <div
            className="channel-header glass"
            style={{
              padding: '16px 24px',
              marginBottom: '24px',
              display: 'flex',
          ...
          >
            <div
              className="upload-zone glass glass-hover"
              style={{
                padding: '40px',
                textAlign: 'center',
