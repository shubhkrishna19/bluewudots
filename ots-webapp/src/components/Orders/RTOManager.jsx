...
import {
    AlertTriangle, ShieldAlert, TrendingDown, CheckSquare, XSquare,
    Truck, PackageX, RotateCcw, Info
} from 'lucide-react';
...
const RTOManager = () => {
    ...
    const rtoOrders = useMemo(() => orders.filter(o => o.status && o.status.startsWith('RTO')), [orders]);
    const totalRTOValue = rtoOrders.reduce((sum, o) => sum + (parseFloat(o.amount || o.totalAmount) || 0), 0);
        ...
        <div className="rto-manager animate-fade">
            ...
            <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex gap-2">
                    ...
                    >
                        📦 RTO Handling <span className="ml-2 bg-black/20 px-1.5 rounded">{rtoOrders.length}</span>
                    </button>
                </div>
            </div>
            ...
            {activeTab === 'RISK_PREVENTION' ? (
                <div className="animate-fade">
                    ...
                    <div className="grid grid-cols-4 gap-4 mb-8">
                        ...
                        <div className="glass p-4 rounded-xl border-l-4 border-orange-500">
                            ...
                            <h3 className="text-2xl font-bold text-orange-400">₹{metrics.capitalAtRisk?.toLocaleString()}</h3>
                        </div>
                    ...
                    <div className="glass rounded-xl overflow-hidden">
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-900/50 text-xs uppercase text-muted font-bold">
                                <tr>
                                    <th className="p-4 w-10"><input type="checkbox" onChange={(e) => setSelectedOrders(e.target.checked ? highRiskOrders.map(o => o.id) : [])} /></th>
                                    ...
                                    highRiskOrders.map(order => (
                                        <tr key={order.id} className={`hover:bg-white/5 transition-colors ${order.risk.level === 'BLOCKED' ? 'bg-purple-500/5' : ''}`}>
                                            ...
                                            <td className="p-4">
                                                ...
                                                <div className="text-[10px] bg-slate-800 px-1 rounded inline-block mt-1">{order.paymentMethod || 'COD'}</div>
                                            </td>
                ...
                <div className="animate-fade">
                    ...
                    <div className="grid grid-cols-4 gap-4 mb-8">
                        {['Customer Refused', 'Incorrect Address', 'Damaged in Transit'].map((reason, i) => (
                            <div key={reason} className="glass p-4 rounded-xl text-center">
                                <h3 className={`text-2xl font-bold ${i === 0 ? 'text-orange-400' : i === 1 ? 'text-red-400' : 'text-blue-400'}`}>
                    ...
                    <div className="rto-list space-y-4">
                        {rtoOrders.length === 0 ? (
                            <div className="glass p-12 text-center"><PackageX className="w-12 h-12 mx-auto mb-4 opacity-20" /><p className="text-muted">No active RTOs to process.</p></div>
                        ) : (
                            rtoOrders.map(rto => (
                                <div key={rto.id} className="glass glass-hover p-6 rounded-xl border-l-4 border-l-orange-500 cursor-pointer transition-all" onClick={() => setSelectedRTO(selectedRTO?.id === rto.id ? null : rto)}>
                                    <div className="flex justify-between items-start">
                                        <div className="flex gap-4">
                                            <div className="p-3 bg-slate-800 rounded-lg"><RotateCcw className="text-orange-500" /></div>
                                            <div>
                                                <h3 className="font-bold text-lg">{rto.id}</h3>
                                                <p className="text-sm text-muted">{rto.customer} • {rto.city}</p>
                                                <span className="inline-block mt-2 text-xs bg-orange-500/10 text-orange-400 border border-orange-500/20 px-2 py-0.5 rounded">{rto.reason || 'Return Initiated'}</span>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <p className="font-bold text-xl">₹{parseFloat(rto.amount || rto.totalAmount || 0).toLocaleString()}</p>
                                            <p className="text-xs text-muted">Since: {new Date(rto.lastUpdated).toLocaleDateString()}</p>
                                        </div>
                                    </div>
                                    {selectedRTO?.id === rto.id && (
                                        <div className="mt-6 pt-6 border-t border-white/5 flex gap-4 animate-fade">
                                            ...
                                            <button className="btn-primary glass-hover flex-1" onClick={(e) => { e.stopPropagation(); handleRTOAction(rto.id, 'reattempt'); }}>🔄 Re-attempt</button>
                                            <button className="btn-secondary glass-hover flex-1" onClick={(e) => { e.stopPropagation(); handleRTOAction(rto.id, 'refund'); }}>💰 Refund</button>
                                            <button className="btn-secondary glass-hover w-12 flex items-center justify-center" onClick={(e) => { e.stopPropagation(); handleRTOAction(rto.id, 'writeoff'); }}>❌</button>
                                        </div>
                                    )}
                                </div>
                            ))
