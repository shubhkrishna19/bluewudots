  ...
  useEffect(() => {
    ...
    const onScanSuccess = async (decodedText) => {
      ...
      if (order) {
        ...
        setScanLog((prev) => [
          { id: order.id, time: new Date().toLocaleTimeString(), status: 'OK' },
          ...prev,
        ])
    ...
    <div className="dispatcher-view animate-fade">
      <div className="section-header">
        ...
        >
          ...
          <div
            className="hub-controls glass"
            style={{ padding: '12px 20px', display: 'flex', gap: '20px', alignItems: 'center' }}
          >
            ...
            >
              ...
              />
              <label htmlFor="autoprint" style={{ fontSize: '0.85rem' }}>
                Auto-Print
              </label>
            </div>
          </div>
      ...
      >
        <div className="scanner-container glass" style={{ padding: '32px', textAlign: 'center' }}>
          ...
          {lastScan && (
            <div
              className={`scan-feedback animate-fade ${lastScan.status === 'SUCCESS' ? 'success' : 'danger'}`}
              style={{
                marginTop: '24px',
                padding: '20px',
                borderRadius: '12px',
        ...
        <div
          className="scan-history glass"
          style={{ padding: '24px', maxHeight: '500px', overflowY: 'auto' }}
        >
          ...
          >
            {scanLog.map((log, idx) => (
              ...
              >
                ...
                <span
                  className={`badge ${log.status === 'OK' ? 'badge-success' : 'badge-danger'}`}
                  style={{ padding: '2px 8px', borderRadius: '4px', fontSize: '0.7rem' }}
                >
                  {log.status}
                </span>
              </div>
