...
const ReturnsManager = () => {
    ...
    const getStatusColor = (status) => {
        switch (status) {
            case ReturnStatus.REQUESTED: return 'var(--warning)';
            case ReturnStatus.PICKUP_SCHEDULED: return 'var(--info)';
            case ReturnStatus.RECEIVED: return 'var(--primary)';
            case ReturnStatus.QC_PASSED: return 'var(--success)';
            case ReturnStatus.QC_FAILED: return 'var(--danger)';
        ...
        <div className="returns-manager animate-fade">
            <div className="section-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <div className="flex gap-2">
                    <span className="badge" style={{ background: 'var(--info)' }}>{returns.length} Active</span>
                </div>
            </div>
                    ...
                    >
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <p className="text-sm text-slate-400 mt-1">Order: {rma.orderId} • {rma.customer}</p>
                            </div>
                            <div className="text-right">
                                <p className="text-sm font-bold">{rma.items && rma.items.length > 0 ? rma.items[0].name || rma.items[0].sku : 'Unknown Item'}</p>
                        ...
                        {selectedReturn?.id === rma.id && (
                            <div className="mt-6 pt-4 border-t border-white/10 animate-fade">
                                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Next Actions</h4>
