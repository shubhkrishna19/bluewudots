...
const QuickOrderForm = ({ onClose }) => {
  const { addOrder, skuMaster, getCarrierRates, getCarrierRecommendation, syncOrderToZoho } =
    useData()
  ...
  return (
    <div
      className="quick-order-form glass animate-fade"
      style={{ padding: '32px', maxHeight: '90vh', overflowY: 'auto' }}
    >
      <div
        style={{
          display: 'flex',
      ...
      <div
        className="success-banner animate-fade"
        style={{
          padding: '16px',
      ...
      {errors.length > 0 && (
        <div
          className="error-banner animate-fade"
          style={{
            padding: '16px',
        ...
        >
          <p style={{ fontWeight: '700', marginBottom: '8px', color: 'var(--danger)' }}>
            ⚠️ Please fix the following:
          </p>
          {errors.map((err, i) => (
            <p key={i} style={{ fontSize: '0.85rem', color: 'var(--danger)' }}>
              • {err}
            </p>
          ))}
        </div>
      ...
      <form onSubmit={handleSubmit}>
          ...
          >
            <div className="form-group">
              <label
                ...
                style={{ fontSize: '0.75rem', display: 'block', marginBottom: '8px' }}
              >
                CUSTOMER NAME *
              </label>
              <input
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <div
            className="form-grid"
            style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '16px' }}
          >
            <div className="form-group">
              <label
                className="text-muted"
              ...
              {shippingRates?.map((rate, i) => (
                <div
                  ...
                  style={{
                    padding: '12px',
                    background:
                      formData.carrierId === rate.carrierId
                        ? 'rgba(99, 102, 241, 0.2)'
                        : 'var(--bg-accent)',
                    border:
                      formData.carrierId === rate.carrierId
                ...
                >
                  <p style={{ fontWeight: '700' }}>
                    {rate.carrierLogo} {rate.carrierName}
                  </p>
                  <p style={{ fontSize: '1.1rem', color: 'var(--success)' }}>₹{rate.total}</p>
                  <p className="text-muted" style={{ fontSize: '0.75rem' }}>
                    {rate.estimatedDelivery[0]}-{rate.estimatedDelivery[1]} days
                  </p>
                  {recommendedCarrier?.carrierId === rate.carrierId && (
                    <p style={{ fontSize: '0.6rem', color: 'var(--primary)', marginTop: '4px' }}>
                      {recommendedCarrier.reason}
                    </p>
                  )}
                </div>
              ))}
