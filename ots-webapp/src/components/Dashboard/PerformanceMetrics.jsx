...
const PerformanceMetrics = () => {
    ...
    const avgWeight = orders.length > 0
    ...
    return (
        <div className="performance-metrics animate-fade">
            ...
            {/* Main KPIs */}
            <div className="kpi-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px', marginTop: '32px' }}>
                {metrics.map((m, idx) => (
                    <div key={idx} className="kpi-card glass glass-hover" style={{ padding: '24px', textAlign: 'center' }}>
                        ...
                        <p className="text-muted" style={{ fontSize: '0.75rem', marginTop: '6px' }}>{m.desc}</p>
                    </div>
                ))}
            </div>
            ...
            <div className="status-breakdown glass" style={{ marginTop: '32px', padding: '28px' }}>
                ...
                <div style={{ display: 'flex', gap: '4px', marginTop: '20px', height: '40px', borderRadius: '8px', overflow: 'hidden' }}>
                    <div style={{ flex: pending, background: 'var(--warning)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {pending > 0 && <span style={{ fontSize: '0.8rem', fontWeight: '700' }}>{pending}</span>}
                    </div>
                    <div style={{ flex: inTransit, background: 'var(--info)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {inTransit > 0 && <span style={{ fontSize: '0.8rem', fontWeight: '700' }}>{inTransit}</span>}
                    </div>
                    <div style={{ flex: deliveredOrders, background: 'var(--success)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {deliveredOrders > 0 && <span style={{ fontSize: '0.8rem', fontWeight: '700' }}>{deliveredOrders}</span>}
                    </div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '12px' }}>
                    ...
                    <span className="text-muted" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <span style={{ width: '12px', height: '12px', background: 'var(--info)', borderRadius: '2px' }}></span>
