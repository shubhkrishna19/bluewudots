...
export const selectCancelledOrders = (state) => selectOrdersByStatus(state, 'CANCELLED')
...
export const selectCODOrders = (state) => selectOrdersByPaymentMethod(state, 'COD')
export const selectPrepaidOrders = (state) => selectOrdersByPaymentMethod(state, 'PREPAID')

// Product/Item selectors
export const selectAllProducts = (state) => state?.products?.list || []
export const selectProductById = (state, productId) =>
  state?.products?.list?.find((p) => p.id === productId)
export const selectProductBySKU = (state, sku) => state?.products?.list?.find((p) => p.sku === sku)
...
export const selectInTransitShipments = (state) => selectShipmentsByStatus(state, 'IN_TRANSIT')
export const selectPendingShipments = (state) => selectShipmentsByStatus(state, 'PENDING')
export const selectDeliveredShipments = (state) => selectShipmentsByStatus(state, 'DELIVERED')
...
export const selectFilteredOrders = (state, filters = {}) => {
  ...
  if (filters.startDate)
    results = results.filter((o) => new Date(o.createdAt) >= new Date(filters.startDate))
  if (filters.endDate)
    results = results.filter((o) => new Date(o.createdAt) <= new Date(filters.endDate))
  if (filters.search) {
